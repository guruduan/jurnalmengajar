...
$PAGE->set_url(new moodle_url('/local/jurnalmengajar/kartu_ujiant.php'));
...
if ($action === 'generate' && confirm_sesskey()) {
    ...
    $pdf->Output('kartu_peserta_ujian_' . userdate(time(), '%Y%m%d_%H%M%S') . '.pdf', 'I');
    exit;
}
...
echo html_writer::start_tag('form', ['method'=>'post','action'=>new moodle_url('/local/jurnalmengajar/kartu_ujiant.php'), 'id'=>'form-kartu-ujian']);
